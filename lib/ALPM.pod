...
=item B<noupgrades>* - a list of package names to not upgrade.  Use
C<match_noupgrade> to check if a path matches.

=item B<noextracts>* - a list of package names to not extract.  Use
C<match_noextract> to check if a path matches.
